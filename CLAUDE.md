...
cargo build --release                # release build (apps only — default-members)
...
**Workspace**: 7 library crates under `crates/`, 2 app binaries under `apps/`.
...
| Crate | Purpose |
|-------|---------|
| `zeroclash-core` | All business logic — config, mihomo client, profiles, enhance pipeline, connections, backup, service, system proxy. **No GUI dependencies.** |
...
| `zeroclash-logging` | 20 log type categories, `logging!`/`logging_error!` macros, `flexi_logger`-based. |
...
- Async bridging: `pollster::block_on()` in GPUI's sync `process_commands()`.
- `.exclude/` contains clash-verge-rev source (manually placed, not a submodule).
- License: CC-BY-NC-SA-4.0 (non-commercial).

## Existing automation

Skills available: `/fix-issue` (GitHub issue → PR), `/gpui-design` (shadcn/ui-style GPUI components).
