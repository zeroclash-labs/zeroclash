...
static SUPPORTED_LOCALES: LazyLock<Vec<Cow<'static, str>>> =
    LazyLock::new(|| rust_i18n::available_locales!());
...
fn resolve_supported_language(language: &str) -> Option<Cow<'static, str>> {
    ...
    for i in (1..=segments.len()).rev() {
        ...
        if let Some(alias) = locale_alias(&prefix)
            && let Some(found) = SUPPORTED_LOCALES
                .iter()
                .find(|l| l.eq_ignore_ascii_case(alias))
        {
            return Some(found.clone());
        }
        if let Some(found) = SUPPORTED_LOCALES
            .iter()
            .find(|l| l.eq_ignore_ascii_case(&prefix))
        {
            return Some(found.clone());
        }
    }
