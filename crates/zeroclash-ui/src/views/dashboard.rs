    ...
    div()
        ...
        .child(
            div()
                ...
                .gap(px(SPACE_MD))
                .mb(px(SPACE_MD))
                .child(div().flex_1().child(core_status_card(c, state.core_running, cx)))
                .child(
                    div()
                        .w_full()
                        .child(
...
fn core_status_card(
    ...
    let btn_text = if core_running { "Stop Core" } else { "Start Core" };
    let btn_color = if core_running { c.danger } else { c.success };

    card(c).child(
        div().flex().flex_col().child(section_title(c, "CORE STATUS")).child(
            div()
                .mt(px(SPACE_MD))
                .flex()
                .items_center()
            ...
            div()
                .mt(px(SPACE_MD))
                .bg(if core_running { c.danger_dim } else { c.success_dim })
...
fn system_info_card(c: Colors, state: &AppState) -> impl IntoElement {
    card(c).child(
        div().flex().flex_col().child(section_title(c, "SYSTEM")).child(
            div()
                .mt(px(SPACE_SM))
                .flex()
                .flex_col()
                ...
                .child(info_row(
                    c,
                    "System Proxy",
                    if state.enable_system_proxy { "ON" } else { "OFF" },
                )),
        ),
    )
}
