        ...
        .child(match total == 0 {
            ...
            false => {
                        ...
                        div()
                            ...
                            .child(
                                div()
                                    .flex_1()
                                    .text_color(c.text_primary)
                                    .child(SharedString::from(&conn.host)),
                            )
                            .child(
                                div()
                                    .w(px(50.))
                                    .text_color(c.text_secondary)
                                    .child(SharedString::from(&conn.network)),
                            )
                            .child(div().w(px(100.)).text_color(c.text_muted).child(
                                SharedString::from(match conn.chains.is_empty() {
                                    true => "-".into(),
                                    false => conn.chains.join(" → "),
                                }),
                            ))
                            .child(
                                div()
                                    .w(px(80.))
                                    .text_color(c.text_secondary)
                                    .child(SharedString::from(&conn.rule)),
                            )
                            .child(div().w(px(120.)).child(SharedString::from(format!(
                                "↓{} ↑{}",
                                fmt_speed(conn.download),
                                fmt_speed(conn.upload)
                            ))))
                            .child(
                                div()
                                    .text_color(c.danger)
                                    ...
                                    .on_mouse_up(
                                        MouseButton::Left,
                                        cx.listener(move |this, _e, _w, cx| {
                                            this.push_command(UiCommand::CloseConnection(
                                                cid2.clone(),
                                            ));
                                            cx.notify();
                                        }),
                ...
                if let Some(conn) = state
                    .connections
                    .iter()
                    .find(|c| c.id == state.selected_conn_id.as_deref().unwrap_or_default())
                {
