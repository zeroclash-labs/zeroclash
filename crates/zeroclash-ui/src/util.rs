    ...
    if let Some(obj) = proxies.as_object() {
        for (name, val) in obj {
            if let Some(typ) = val.get("type").and_then(|t| t.as_str()) {
                ...
                let history: Vec<DelayHistory> = val.get("history").and_then(|h| h.as_array()).map(|arr| arr.iter().map(|v| DelayHistory { time: String::new(), delay: v.get("delay").and_then(|d| d.as_u64()).unwrap_or(0) }).collect()).unwrap_or_default();
